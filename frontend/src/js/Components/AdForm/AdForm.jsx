...
const AdForm = () => {
    ...
    useEffect(() => { //тут изменено, чтобы проверить отправку объявлений
    ...
    const handleSubmit = async e => {
        ...
        try {
            await fetch(`/api/v2/announcements/`, {
                method: 'POST',
                body: JSON.stringify({
                    title: puppyParam.title,
                    breed: puppyParam.breed,
                    male_count: parseInt(puppyParam.sexMale),
                    female_count: parseInt(puppyParam.sexFemale),
                    birthday: puppyParam.dateOfBirth,
                    with_documents: data.documents,
                    price: parseInt(puppyParam.price),
                    city: puppyParam.city,
                    is_transportable: data.isTransportable,
                    description: puppyParam.description,
                    pet_photos: formatPhotosToPost(puppyParam.photos),
                    parents_description: puppyParam.parentsDesc,
                    parent_photos: formatPhotosToPost(puppyParam.parentsPhotos),
                }),
                headers: {
